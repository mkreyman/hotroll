...
setup_letsencrypt() {
  ...
  echo "hot-roll.com www.hot-roll.com" > domains.txt
...
create_pems() {
  openssl req -x509 -nodes -days 730 -newkey rsa:1024 -keyout privkey.pem -out fullchain.pem -subj "/C=US/ST=Anystate/L=Anywhere/O=Initech/OU=Software/CN=hot-roll.com"
...
if [ "$CA_SSL" = "true" ]; then
  ...
  cp $SSL_ROOT/certs/hot-roll.com/fullchain.pem $SSL_ROOT/certs/hot-roll.com/privkey.pem $SSL_CERT_HOME
